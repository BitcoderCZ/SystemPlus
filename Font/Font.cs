...
{
    internal class GlyphBitmap : ICloneable
    {
        ...
        public object Clone()
        {
            return new GlyphBitmap(Width, Height, (byte[])Pixels.Clone());
        }

        public GlyphBitmap Cloned()
        {
            return new GlyphBitmap(Width, Height, (byte[])Pixels.Clone());
        }
    }

    internal class FontGlyph : ICloneable
    {
        ...
        public object Clone()
        {
            return new FontGlyph() { Image = Image.Cloned(), };
        }
    }
    ...
    internal class Font : IDisposable
    {
        ...
        private Dictionary<float, Dictionary<char, FontGlyph>> renderedGlyphs;
        ...
        public Font(string _path, int id)
        {
            if (!File.Exists(_path))
                throw new FileNotFoundException($"File {_path} not found!", _path);

            DateTime start = DateTime.Now;
            Console.WriteLine("Loading " + _path);
            ...
            Console.WriteLine("Loaded " + _path + " in " + (DateTime.Now - start).TotalSeconds + " seconds");
        }
        ...
        public FontGlyph RenderGlyph(char ID, float scale)
        {
            ...
            /*if (renderedGlyphs.ContainsKey(scale) && renderedGlyphs[scale].ContainsKey(ID))
                    return renderedGlyphs[scale][ID].Cloned();*/

            FontGlyph glyphTarget = new FontGlyph();
            ...
            /* if (!renderedGlyphs.ContainsKey(scale))
                    renderedGlyphs.Add(scale, new Dictionary<char, FontGlyph>());

                if (!renderedGlyphs[scale].ContainsKey(ID))
                    renderedGlyphs[scale].Add(ID, glyphTarget.Cloned());*/
