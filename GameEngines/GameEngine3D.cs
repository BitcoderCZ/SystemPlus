...
namespace SystemPlus.GameEngines
{
    public abstract class GameEngine3D
    {
        ...
        public GameEngine3D engine { get { return this; } }

        // For: initialize file -> allows to directly modify console buffer
        ...
        public abstract StartInfo3D Start();
        public abstract void Update(float deltaTime, int miliseconds);
        /// <summary>
        /// Used for drawing (mainly UI)
        /// </summary>
        ...
        public void GameStart()
        {
            ...
            FOV = info.FOV;
            Z0 = (bufferSize.x / 2f) / (float)Math.Tan((FOV / 2f) * 3.14159265f / 180f);
        ...
        private Point3D ApplyPerspective(Point3D original)
        {
            Point3D toReturn;
        ...
        {
            Point3D toReturn;
            toReturn.x = original.x + bufferSize.x / 2d;
            toReturn.y = original.y + bufferSize.y / 2d;
            toReturn.z = original.z;
        ...
        {
            if (InUpdate)
                throw new Exception("Cant draw in Update, Use LateUpdate Instead");
            if (!PixOneByOne)
            {
        ...
        public void DrawSquare(int x1, int y1, int x2, int y2, short pixelType = 0x2588, short color = 0x000F)
        {
            DrawLine(x1, y1, x2, y1, pixelType, color); // ---
            DrawLine(x2, y1, x2, y2, pixelType, color); //   |
            DrawLine(x1, y2, x2, y2, pixelType, color); // ___
            DrawLine(x1, y1, x1, y2, pixelType, color); // |
        }
        public void DrawSquare(int x1, int y1, int x2, int y2,
            ...
            => DrawSquare(x1, y1, x2, y2, (short)pixelType, (short)color);
        public void DrawSquare(Vector2Short pos1, Vector2Short pos2, short pixelType = 0x2588, short color = 0x000F)
            => DrawSquare(pos1.x, pos1.y, pos2.x, pos2.y, pixelType, color);
        public void DrawSquare(Vector2Short pos1, Vector2Short pos2,
            PIXEL pixelType = PIXEL.PIXEL_SOLID, CONSOLE_COLOR color = CONSOLE_COLOR.White)
            => DrawSquare(pos1, pos2, (short)pixelType, (short)color);
    ...
    public class StartInfo3D
    {
        ...
        public StartInfo3D(string _title, short _width, short _height, short _fontSize, float _FOV = 45f, bool _pixelsOneByOne = true)
        {
            Title = _title;
            if (_width <= 0)
